...
while True:
    ...
    if current_frame_has_face:
        ...
        if elapsedTime > 4 and signIn is not None:
            ...
            producer.send(TOPIC,payload)
            store.append({'activeTime': activeTime, 'signIn': signIn.strftime("%I:%M:%S")})
            #signIn = None
            #delta = None
            #signOut = None
            #flag = True
...
totalTime = (int)(floor((endTime - signIn).total_seconds()))
activeTime = (int)(floor(activeTime))
store = {'startTime': signIn.strftime("%I:%M:%S"), 'endTime': endTime.strftime("%I:%M:%S"), 'activeTime': str(activeTime), 'totalTime':str(totalTime)}
